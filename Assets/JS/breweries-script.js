...
var documentApiKeys = [
  "f2891f69c0b5f586001e03d7f04150cd",
  "5c84bf0ba620e31313ba63c7c9b54297",
  "1d72ed71331751f36558c92ff7f8a0cf",
  "e59184fab7b4752cf7593f7ca4a627f1",
  "442e928049c9bb7b553d48b27474017a",
  "bb8246f243790c635ad142fe7f2030ba",
  "aeb59f84ae3c5fc239e52dd74a168c50",
];
var selectedKey =
  ...
  fetch(openBreweryDBUrl)
    ...
    .then(function (data) {
      for (var i = 0; i < data.length; i++) {
        breweryApiData = data;
        let template = `
        <div class="rounded-2xl overflow-hidden flex flex-shrink-0 min-w-1/4 content-between font-mono hover:scale-105 z-10 bg-coverfont-extrabold bg-[url('./Assets/pictures/BeerSuds.jpg')]">

          <div class="py-2.5 px-2.5 text-left card-shadow justify-between hover:bg-indigo-400 hover:bg-opacity-50 w-72 text-left brewCard">

            <div class="font-extrabold text-xl h-1/4 mb-2 font-['Bungee_Inline']">${
              data[i].name
            }</div>
    ...
    if (response.status === 200) {
      response.json().then(function (tacoData) {
        ...
        for (var i = 0; i < tacoApiData.length; i++) {
          ...
          <div class="rounded-2xl overflow-hidden flex flex-shrink-0 min-w-1/4 content-between font-mono hover:scale-105 bg-cover bg-[url('./Assets/pictures/Tacos.jpg')]">
          ...
          if (tacoApiData[i].restaurant_website != null) {
            tacoCardTemplate += websiteAdd(
              tacoApiData.data[i].restaurant_website
            );
          }
...
function getTacoMenu() {
  ...
  for (let i = 0; i < tacoMenu.length; i++) {
    ...
    var menuItem = document.createTextNode(
      `${tacoMenu[i].name}     $${tacoMenu[i].price}`
    );
